...
Hermes connects to WeChat personal accounts via Tencent's official **iLink Bot API** (`ilinkai.weixin.qq.com`), not a reverse-engineered protocol.

**Dependencies:**
```bash
pip install aiohttp cryptography certifi
```
The adapter auto-checks `AIOHTTP_AVAILABLE` and `CRYPTO_AVAILABLE` before activating.

**Config (in `config.yaml` or env vars):**
```yaml
platforms:
  weixin:
    enabled: true
    token: "your_ilink_bot_token"       # or WEIXIN_TOKEN env var
    extra:
      account_id: "your_account_id"      # or WEIXIN_ACCOUNT_ID
      base_url: "https://ilinkai.weixin.qq.com"  # default, usually no change
      cdn_base_url: "https://novac2c.cdn.weixin.qq.com/c2c"  # default
```

**Setup flow:**
1. Register an iLink Bot at https://developers.weixin.qq.com/ilink/bot/ (requires WeChat Open Platform account)
2. Create a bot and obtain its Token + Account ID
3. Run `hermes gateway setup` (includes QR code login helper — scan with WeChat to authorize the bot)
4. Run `hermes gateway run` to start

**Capabilities:**
- Text messages (send + receive, with Markdown→WeChat-friendly formatting)
- Images, videos, audio (`.silk`), files via AES-128-ECB encrypted CDN
- Group chats (auto-detected via `room_id` / `chat_room_id` fields)
- DMs with context tokens for conversation continuity
- Typing indicators (via `sendtyping` endpoint)
- Message deduplication (5-min TTL window)
- Session expiry auto-recovery (errcode -14 and ret=-2/"unknown error" triggers reconnect)

**Cron/one-shot delivery** is supported via `send_weixin_direct()` in the adapter; uses `WEIXIN_TOKEN`, `WEIXIN_ACCOUNT_ID`, `WEIXIN_BASE_URL`, and `WEIXIN_CDN_BASE_URL` env vars when called outside the gateway lifecycle.

**Code reference:** `gateway/platforms/weixin.py` (~2170 lines). Implementation details (API endpoints, error codes, media constants) in `references/weixin-ilink-api.md`.
...
### Desktop app not launching after build

After `hermes desktop` builds successfully, Electron's Linux `chrome-sandbox`
needs setuid root. If you see `✗ Failed to configure Electron's Linux sandbox helper`,
run in terminal:
...
Then `hermes desktop --skip-build` to launch. One-time per build.

### Cron job on a different profile

The `cronjob` tool operates on the current profile. To create a job under another
profile, use CLI: `hermes --profile <name> cron create "schedule" "prompt" --name "..." --deliver local`.
The prompt MUST come right after schedule, before any `--flags`.
`enabled_toolsets` can't be set via CLI — edit `~/.hermes/profiles/<name>/cron/jobs.json` directly.
...
- **Gateway crash loop with "Another gateway instance is already running"**: A stale gateway process left a lock file. Kill the stale PID and remove the lock: `kill -9 <PID> && rm -f ~/.hermes/.gateway.lock && hermes gateway restart`. This happens when the previous gateway process crashed or was SIGKILL'd without cleaning up.
